...
abstract class Model
{
    ...
    {
        $data = [];
        $data['list'] = self::pageData($where, $page, $count, $order);
     ...
     * @return array
     */
    public static function pageData($where=[], $page=1, $count=10, $order='', $cache=false)
    {
        return self::where($where)->cache($cache)
                ->page($page, $count)->order($order)
                ->asArray()->select();
    }
     ...
     * @return mixed
     */
    public static function findAll($where=[], $fields=[], $order='', $cache=false)
    {
        return self::field($fields)->where($where)->order($order)->cache($cache)->asArray()->select();
    }
